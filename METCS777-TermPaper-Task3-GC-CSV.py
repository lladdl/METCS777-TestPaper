...
if len(sys.argv) != 3:
    print("Usage: taxi_top_taxis.py <input_file_path> <output_dir>", file=sys.stderr)
    sys.exit(-1)
...
sc = SparkContext(appName="TaxiAnalysisRDD-TopTaxis")
...
medallion_license = fields.map(lambda f: (f[0], f[1]))

top_taxis = (medallion_license.distinct()
             .map(lambda x: (x[0], 1))
             .reduceByKey(lambda a, b: a + b))

top10 = top_taxis.takeOrdered(10, key=lambda x: -x[1])
...
summary.extend([(taxi, count) for taxi, count in top10])
summary.extend([
    ("--- Economic Metrics ---", ""),
...
print(f"\n✅ Job complete: Top taxi = {top10[0][0]} ({top10[0][1]} unique licenses)")

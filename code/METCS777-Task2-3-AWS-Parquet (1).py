...
"""
AWS EMR/YARN-safe PySpark script (DataFrame API, no RDD parse_line)
...
Usage (on EMR/YARN):
  spark-submit taxi_tasks_df_aws.py s3://bucket/path/input/ s3://bucket/path/out/ --format parquet
  spark-submit taxi_tasks_df_aws.py s3://bucket/path/input.csv.bz2 s3://bucket/path/out/ --format csv

Notes:
  - Provide --format explicitly (csv|parquet) for reliability.
  - Make sure your IAM/EMR role has S3 read/write permissions.
"""
...
def parse_args():
    ...
    p.add_argument("input_path", help="S3 path to input (e.g., s3://bucket/data/ or file)")
    p.add_argument("output_dir", help="S3 path to output dir (e.g., s3://bucket/out)")
    p.add_argument("--format", choices=["csv", "parquet"], required=True,
                   help="Input format (csv or parquet)")
    p.add_argument("--csv-header", action="store_true", default=True,
                   help="CSV has header row (default: False)")
    p.add_argument("--csv-infer-schema", action="store_true", default=True,
...
def make_spark():
    spark = (
        ...
        .appName("TaxiTasks-DF-AWS")
        # Conservative YARN-safe defaults; tune to your cluster size
...
# ---------------------- I/O ----------------------
def load_df(spark, input_path, fmt, csv_header=False, csv_infer=True):
    if fmt == "parquet":
...
def estimate_vcpus(sc):
    try:
        ...
        # add 2 for driver as rough default; adjust as needed for your cluster
...
def save_summary(sc, output_dir, rows):
    ...
    rdd.saveAsTextFile(output_dir.rstrip("/") + "/results_summary")
...
def main():
    ...
    start = time.time()
    print(f"Reading {args.input_path} as {args.format.upper()} …")

    raw = load_df(
        spark, args.input_path, args.format,
        csv_header=args.csv_header, csv_infer=args.csv_infer_schema
    )
    ...
    # Summary tuples (print-like text output)
    summary = [
        ("--- Job & Cluster ---", ""),

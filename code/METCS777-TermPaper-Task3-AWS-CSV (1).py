...
# -*- coding: utf-8 -*-
"""
Task 2 only: Top 10 drivers by dollars per minute (EMR/YARN-safe)

- RDD-style CSV load (textFile → drop header → split(','))
- Auto schema for generic _c* (supports classic21/20/17) or --schema-hint
- Cleans & filters, runs Task 2, writes JSON + text summary
"""
...
def make_spark():
    spark = (
        ...
        .appName("Taxi-Task2-TopDrivers-CSV-RDDLoader")
...
def main():
    ...
    try:
        print(f"[DEBUG] Task2 start | input={args.input_path}")
        ...
        t0 = time.time()
        drivers = (df.groupBy("hack_license")
        ...
        # Metrics + summary (printable text)
        end = time.time()
        runtime = end - start
        ...
        summary = [
            ...
            ("Output_JSON", f"{outbase}/task2_top_drivers_10"),
        ]
        save_summary(sc, args.output_dir, summary)
        print(f"Wrote Task2 JSON to {outbase}/task2_top_drivers_10 and summary to {outbase}/results_summary")

    except Exception:
        print("[ERROR] Unhandled exception in Task2:", file=sys.stderr)

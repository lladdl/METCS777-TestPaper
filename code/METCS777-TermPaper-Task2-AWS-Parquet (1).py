...
# -*- coding: utf-8 -*-
"""
EMR/S3-safe CSV➜Parquet staging + Task 1 (Top 10 medallions by # distinct drivers)
Saves:
  <out>/task1_top_taxis_10/   (JSON)
...
if len(sys.argv) not in (3, 4):
    print("Usage: taxi_csv_to_parquet_task1_top_taxis.py <input_csv_s3_path> <output_dir_s3> [staging_parquet_s3_path]",
          file=sys.stderr)
    sys.exit(-1)

csv_path = sys.argv[1]
out_dir  = sys.argv[2]
stage_parquet = sys.argv[3] if len(sys.argv) == 4 else f"{out_dir.rstrip('/')}/staging_parquet_task1"

# --- Spark ------------------------------------------------------------
spark = (
    SparkSession.builder
    .appName("Taxi-CSV-to-Parquet-Task1-EMR-SAFE")
...
# --- Read staged Parquet & prepare for Task 1 ------------------------
df = spark.read.parquet(stage_parquet).repartition(4).cache()
total_records = df.count()
filtered = (
    df.select("medallion","hack_license","trip_time_in_secs","total_amount")
...
# --- Task 2 -----------------------------------------------------------
t1_start = time.time()
top_taxis = (filtered.groupBy("medallion")
...
summary = [
    ...
    ("Output JSON", f"{outbase}/task1_top_taxis_10"),
]
